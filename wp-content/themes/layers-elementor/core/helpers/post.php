	...
	function layers_post_class( $classes ) {
		...
		if( ( isset( $woocommerce ) && is_cart() && 'product' == get_post_type() ) || is_post_type_archive( 'product' ) || is_tax( 'product_cat' ) || is_tax( 'product_tag' ) ) {
			...
			if( !class_exists( 'Layers_WooCommerce' ) ) {
				$wc_span = 12 / wc_get_loop_prop( 'columns' );
			} else {
				$wc_span = 12 / apply_filters( 'loop_shop_columns', 3 );
			}
			// Set the allowed spans 
...
/**
* Set the correct page template when adding a new Elementor page
*
* @param int $postid ID for post
*
*/
function layers_set_elementor_page_template( $mid, $object_id, $meta_key, $_meta_value ) {

    if ( '_elementor_edit_mode' !== $meta_key || 'builder' !== $_meta_value ) {
        return;
    }

    $page_template_meta_key = '_wp_page_template';
    $page_template = get_post_meta( $object_id, $page_template_meta_key, true );
    if ( empty( $page_template ) ) {
        update_post_meta( $object_id, $page_template_meta_key, 'template-blank.php' );
    }
}

add_action( 'added_post_meta', 'layers_set_elementor_page_template', 10, 4 );