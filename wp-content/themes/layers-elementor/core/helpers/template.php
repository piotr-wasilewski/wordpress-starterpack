...
// 	// Bail if not Layers.
// 	$theme = wp_get_theme();
// 	if ( 'layers-elementor' !== $theme->template ) return;
...
/**
 * Get the correct Elementor Activate or Install button
 *
 * @param string $page for template php name
 * @return Post Object
 *
 */
if( !function_exists( 'layers_get_elementor_install_link' ) ){
	function layers_get_elementor_install_link(){
		global $plugins;
		...
		// Get Plugins
		$plugins = get_plugins();
		$elementor_pro_plugin_slug = 'elementor-pro/elementor-pro.php';
		$elementor_plugin_slug = 'elementor/elementor.php';

		if( isset( $plugins[ $elementor_pro_plugin_slug ] ) ) : 
			$elementor_pro_exists = TRUE;
		else :
			$elementor_pro_exists = FALSE;
		endif;

		if( isset( $plugins[ $elementor_plugin_slug ] ) ) :
			$elementor_exists = TRUE;
			$elementor_install_label = 'Activate Elementor';
			$elementor_activation_url = wp_nonce_url( 'plugins.php?action=activate&plugin=' . $elementor_plugin_slug, 'activate-plugin_' . $elementor_plugin_slug );
		else :
			$elementor_exists = FALSE;
			$elementor_install_label = 'Install Elementor';
			$elementor_activation_url = wp_nonce_url( add_query_arg( array( 'action' => 'install-plugin', 'plugin' => 'elementor' ), admin_url( 'update.php' ) ), 'install-plugin_elementor' );
		endif;

		$elementor_params = array(
			'exists' => $elementor_exists,
			'pro_exists' => $elementor_pro_exists,
			'label' => $elementor_install_label,
			'url' => $elementor_activation_url
		);


		return $elementor_params;
	}
}